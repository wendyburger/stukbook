class UsersController < ApplicationController
  ...
  def index
    ...
    when "requests"
      @users = current_user.pending_friendship_request_from.map(&:user)
    when "pending"
      @users = current_user.pending_friendship_request_to.map(&:friend)
    else
  ...
  def get_counts
    ...
    @pending_count = current_user.pending_friendship_request_to.map(&:friend).size
  end
